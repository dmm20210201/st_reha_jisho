...
RSpec.describe 'Userモデルのテスト', type: :model do
  describe 'バリデーションのテスト' do
    ...
    context 'affiliationカラム' do
      it '20文字以下であること: 20文字は〇' do
        user.affiliation = Faker::Lorem.characters(number: 20)
        is_expected.to eq true
      end
      it '20文字以下であること: 21文字は×' do
        user.affiliation = Faker::Lorem.characters(number: 21)
        is_expected.to eq false
      end
    end
  ...
  describe 'アソシエーションのテスト' do
    context 'Rehabilitaionモデルとの関係' do
      it '1:Nとなっている' do
        expect(User.reflect_on_association(:rehabilitations).macro).to eq :has_many
      end
    end
  end
end